...
export class StereoRenderer {
  ...
  _updateStereoCameras() {
    ...
    // 获取相机右方向
    right
      .crossVectors(new THREE.Vector3(0, 0, -1).applyQuaternion(this.camera.quaternion), up)
      .normalize();
    ...
    const target = this.camera.position
      .clone()
      .add(new THREE.Vector3(0, 0, -this.focalLength).applyQuaternion(this.camera.quaternion));
